...
export function RenderDeck({ parentSelector }) {
  ...
  function render(collectionStore: CollectionStoreType, store: ThingStoreType, activeDeckStore: ThingStoreType) {
    var deck: Deck = store.get();
    const isActive = activeDeckStore.get().deck === store.get().id;

    var parentSel = select(parentSelector);
    parentSel.classed('selected', isActive);

    establish(parentSel, 'button', `#deck-${deck.id}`, curry(initActiveButton)('deck'));

    var nameSel = establish(parentSel, 'div', `#title-${deck.id}`, curry(initEditable)('title'));
    nameSel.text(deck.title);
    
    var descSel = establish(parentSel, 'div', `#text-${deck.id}`, curry(initEditable)('text'));
    ...
    function initActiveButton(prop: string, sel) {
      ...
      function setProp() { activeDeckStore.setPart({ [prop]: store.get().id });
      }
    } 
